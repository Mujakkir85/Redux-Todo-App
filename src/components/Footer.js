...
export default function Footer() {
    ...
    const handleColorChnaged = (color) => {
        ...
        <div className="mt-4 flex justify-between text-xs text-gray-500">
            ...
            <ul className="flex space-x-1 items-center text-xs">
                ...
                <li className={`cursor-pointer ${status === "Complete" && "font-bold"}`}
                 onClick = {()=> handleStatusChanged('Complete')}>Complete</li>
                <li></li>
                <li></li>
                <li className={`h-3 w-3 border-2 border-green-500 md:hover:bg-green-500 
                rounded-full cursor-pointer ${ colors.includes('green') && "bg-green-500"}`}
                 onClick={ () => handleColorChnaged('green') }></li>

                <li className={`h-3 w-3 border-2 border-red-500 md:hover:bg-red-500 
                rounded-full cursor-pointer ${colors.includes('red') && "bg-red-500"} `}
                 onClick={ () => handleColorChnaged('red') }></li>
                 
                <li className={`h-3 w-3 border-2 border-yellow-500 md:hover:bg-yellow-500 
                rounded-full cursor-pointer ${colors.includes('yellow') && "bg-yellow-500"}`}
                 onClick={ () => handleColorChnaged('yellow') }></li>
            </ul>
        </div>
    );
}
